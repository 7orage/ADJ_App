...
export default function EquipierResidentsScreen({ onNavigate, residents, onOuvrirResident }) {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          {filtres.map(r => (
            <TouchableOpacity key={r.id} style={styles.carte} onPress={() => onOuvrirResident(r)}>
              <View style={styles.avatar}>
                {r.photo
                  ? <Image source={{ uri: r.photo }} style={styles.avatarImg} />
                  : <Ionicons name="person" size={22} color="#fb7500" />
                }
              </View>
...
const styles = StyleSheet.create({
  ...
  avatar: {
    ...
    backgroundColor: '#fff3e8', borderWidth: 2, borderColor: '#fb7500',
    alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
  },
  avatarImg: { width: 48, height: 48, borderRadius: 24 },
  carteTexte: { flex: 1 },
  carteNom: { fontSize: 17, fontWeight: '600', color: '#333' },
  carteSub: { fontSize: 13, color: '#fb7500', marginTop: 2 },
});
