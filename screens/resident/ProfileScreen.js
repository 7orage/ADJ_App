...
export default function ProfileScreen({ onNavigate, resident, onSauvegarder, onDeconnexion }) {
  const [photo, setPhoto] = useState(resident?.photo || null);
  ...
  const choisirPhoto = async () => {
    ...
    if (!result.canceled) {
      ...
      onSauvegarder?.({ ...resident, photo: nouvellePhoto });
    }
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.profileTop}>
          <TouchableOpacity style={styles.avatarWrapper} onPress={choisirPhoto}>
            {photo
              ...
              : <View style={styles.avatar}>
                  <Ionicons name="person" size={46} color={COULEUR} />
                </View>
            }
          ...
          <Text style={styles.name}>{resident.prenom} {resident.nom}</Text>
        </View>

        <View style={styles.card}>
          <Ionicons name="calendar-outline" size={28} color={COULEUR} style={styles.cardIcon} />
          <View style={styles.cardText}>
            <Text style={styles.cardLabel}>Date d'inscription</Text>
            <Text style={styles.cardValue}>{resident.dateInscription}</Text>
          </View>
        </View>

        <View style={styles.card}>
          <Ionicons name="sunny-outline" size={28} color={COULEUR} style={styles.cardIcon} />
          <View style={styles.cardText}>
            <Text style={styles.cardLabel}>Jours inscrits</Text>
            <Text style={styles.cardValue}>{resident.jours.join(' · ')}</Text>
          </View>
        </View>

        <View style={styles.card}>
          <Ionicons name="document-text-outline" size={28} color={COULEUR} style={styles.cardIcon} />
          <View style={styles.cardText}>
            <Text style={styles.cardLabel}>Dispositifs mis en place (PIA)</Text>
            <Text style={styles.cardValue}>{resident.pia}</Text>
            {resident.piaMaj ? <Text style={styles.cardSub}>Mis à jour le {resident.piaMaj}</Text> : null}
          </View>
        </View>

        <View style={styles.card}>
          <Ionicons name="call-outline" size={28} color={COULEUR} style={styles.cardIcon} />
          <View style={styles.cardText}>
            ...
            <Text style={styles.phone}>{resident.contactTel}</Text>
            <View style={styles.divider} />
            <Text style={styles.cardLabel}>Transport</Text>
            <Text style={styles.cardValue}>{resident.transport}</Text>
            <Text style={styles.phone}>{resident.transportTel}</Text>
          </View>
        </View>

        <View style={styles.card}>
          <Ionicons name="pricetag-outline" size={28} color={COULEUR} style={styles.cardIcon} />
          <View style={styles.cardText}>
            <Text style={styles.cardLabel}>Prix à la journée</Text>
            <Text style={styles.cardValue}>{resident.prix}</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  header: {
    height: 90, backgroundColor: COULEUR,
  ...
  profileTop: { alignItems: 'center', marginBottom: 6, gap: 6 },
  avatarWrapper: { position: 'relative' },
  avatar: {
    width: 100, height: 100, borderRadius: 50,
    backgroundColor: '#fff', borderWidth: 3, borderColor: COULEUR,
    alignItems: 'center', justifyContent: 'center',
  },
  avatarImg: { width: 100, height: 100, borderRadius: 50, borderWidth: 3, borderColor: COULEUR },
  avatarBadge: {
    position: 'absolute', bottom: 0, right: 0,
    width: 28, height: 28, borderRadius: 14,
    backgroundColor: COULEUR, alignItems: 'center', justifyContent: 'center',
  ...
  phone: { fontSize: 16, color: '#aaa', marginTop: 4 },
  divider: { height: 1, backgroundColor: '#f0f0f0', marginVertical: 12 },
  boutonDeconnexion: {
    flexDirection: 'row', alignItems: 'center', gap: 10,
    borderWidth: 1.5, borderColor: '#ef4444', borderRadius: 14,
    paddingVertical: 14, justifyContent: 'center', marginBottom: 20,
  },
